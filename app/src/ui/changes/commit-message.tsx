...
> {
  ...
  private onSummaryChanged = (summary: string) => {
    this.setState({
      commitMessage: {
        ...
        // that the commit message was not AI generated (anymore).
        generatedByAi: false,
        timestamp: Date.now(),
      },
    })
  ...
  private onAIButtonClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault()
  ...
  private renderAIButton() {
    if (!this.isAIButtonEnabled) {
      return null
    }
    ...
    let ariaLabel = 'Generate commit message with AI'
  ...
  private renderCommitOptionsButton() {
    ...
    return (
      <>
        {(this.isCoAuthorInputEnabled || this.isAIButtonEnabled) && (
  ...
  /**
   * Whether the AI button should be available.
   *
   * The button is always shown when the dispatcher exposes the generate
   * callback, regardless of whether the current accounts have a Copilot
   * license. This lets users with OpenAI-compatible or external CLI
   * providers still see and use the button. Clicking it without any
   * available provider surfaces a friendly error pointing to
   * Preferences → AI Summary.
   */
  private get isAIButtonEnabled() {
    const { onGenerateCommitMessage } = this.props
    return onGenerateCommitMessage !== undefined
  }

  /**
   * Whether an in-flight commit message generation can be cancelled.
   * Any provider (Copilot, OpenAI-compatible, external CLI) supports
   * cancellation, so this only depends on the dispatcher exposing the
   * cancel callback.
   */
